...
class TZXproccesor
{
    ...
    int CURRENT_LOADING_BLOCK = 0;
     ...
     bool isHeaderTZX(File32 tapFileName)
     {
        if (_mFile != 0)
        {
          ...
          // ERROR ERRROR ERROR ERROR ERROR ERRROR ERROR ERROR
          // ERROR ERRROR ERROR ERROR ERROR ERRROR ERROR ERROR

          byte* bBlock = (byte*)calloc(10+1,sizeof(byte));
          bBlock = _sdm.readFileRange32(tapFileName,0,10,true);
     ...
     bool isFileTZX(File32 tapFileName)
     {
         if (_mFile != 0)
         {

            // Capturamos el nombre del fichero en szName
            String szNameStr = _sdm.getFileName(tapFileName);
    ...
    void getInfoFileTZX(char* path) 
    {
      ...
      LAST_MESSAGE = "Analyzing TZX file";
