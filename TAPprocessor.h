    ...
    private:
        ...
        bool getBlockDescriptor(File32 mFile, int sizeTAP)
        {
            ...
            while(reachEndByte==false && sizeB!=0)
            {
                ...
                uint8_t* ptr = (uint8_t*)ps_calloc(sizeB-1+1,sizeof(uint8_t));
                ptr = sdm.readFileRange32(_mFile,startBlock,sizeB-1,false);
                ...
                uint8_t* ptr2 = (uint8_t*)ps_calloc(1+1,sizeof(uint8_t));
                ptr2 =  sdm.readFileRange32(_mFile,startBlock+sizeB-1,1,false); 
                blockChk =ptr2[0];         
                free(ptr2);
                ...
                if (blockChk == chk)
                {
                    ...
                    // 0x03 - CODE FILE
                    uint8_t* ptr2 = (uint8_t*)ps_calloc(1+1,sizeof(uint8_t));
                    ptr2 = sdm.readFileRange32(_mFile,startBlock+1,1,false);
                    int typeBlock = ptr2[0];
                    free(ptr2);
                    ...
                    startBlock = startBlock + sizeB;
                    // Tamaño
                    uint8_t* ptr_1 = (uint8_t*)ps_calloc(1+1,sizeof(uint8_t));
                    ptr_1 = sdm.readFileRange32(_mFile,startBlock+1,1,false);
                    uint8_t* ptr_2 = (uint8_t*)ps_calloc(1+1,sizeof(uint8_t));
                    ptr_2 = sdm.readFileRange32(_mFile,startBlock,1,false);
                    newSizeB = (256*ptr_1[0]) + ptr_2[0];
                    free(ptr_1);
                    free(ptr_2);  
            ...
            {         
                    ...
                    for (int i = m; i < _myTAP.numBlocks; i++) 
                    {
                        ...
                        if (_myTAP.descriptor[i].type == 0) 
                        {
                            ...
                            uint8_t* bufferPlay = (uint8_t*)ps_calloc(_myTAP.descriptor[i].size+1,sizeof(uint8_t));
                        ...
                        else 
                        {
                            ...
                            if (_myTAP.descriptor[i].size > blockSizeSplit)
                            {
                                ...
                                for (int n=0;n < blocks;n++)
                                {
                                    ...
                                    uint8_t* bufferPlay = (uint8_t*)ps_calloc(blockSizeSplit+1,sizeof(uint8_t));
