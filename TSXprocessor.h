...
class TSXprocessor
{
    ...
    bool isHeaderTSX(File32 TSXFile)
    {
      if (TSXFile != 0)
      {
          ...
          bBlock = sdm.readFileRange32(TSXFile,0,10,false); 
    ...
    int getWORD(File32 mFile, int offset)
    {
        ...
        ptr1 = sdm.readFileRange32(mFile,offset+1,1,false);
        uint8_t* ptr2 = (uint8_t*)ps_calloc(1+1,sizeof(uint8_t));
        ptr2 = sdm.readFileRange32(mFile,offset,1,false);
    ...
    int getBYTE(File32 mFile, int offset)
    {
        ...
        ptr = sdm.readFileRange32(mFile,offset,1,false);
        sizeB = ptr[0];
        free(ptr);
   ...
   void analyzeID53(File32 mFile, int currentOffset, int currentBlock)
    ...
    void playBlock(tTSXBlockDescriptor descriptor)
    {
        ...
        else
        {
            ...
            uint8_t* bufferPlay = (uint8_t*)ps_calloc(descriptor.size+1,sizeof(uint8_t));
                        ...
                        switch (_myTSX.descriptor[i].ID)
                        {
                            ...
                            case 20:
                              ...
                              else
                              {
                                  ...
                                  if (!STOP && !PAUSE)
                                  {
                                      uint8_t* bufferPlay = (uint8_t*)ps_calloc(_myTSX.descriptor[i].size+1,sizeof(uint8_t));
