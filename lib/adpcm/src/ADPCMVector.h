...
class ADPCMVector {
 public:
  ...
  /// Move constructor
  ADPCMVector(ADPCMVector<T> &&moveFrom) {
    swap(moveFrom);
    moveFrom.clear();
  };

  /// Move operator
  ADPCMVector &operator=(ADPCMVector &&moveFrom) {
    swap(moveFrom);
    moveFrom.clear();
    return *this;
  }
  ...
  /// copy constructor
  ADPCMVector(ADPCMVector<T> &copyFrom) {
  ...
  /// copy operator
  ADPCMVector<T> &operator=(ADPCMVector<T> &copyFrom) {
    resize_internal(copyFrom.size(), false);
    for (int j = 0; j < copyFrom.size(); j++) {
      p_data[j] = copyFrom[j];
    }
