...
#define DEF(type, name, bytes, read, write)                                    \
  ...
  static av_always_inline type bytestream2_peek_##name(GetByteContext *g) {    \
    if (g->buffer_end - g->buffer < bytes) return 0;                           \
...
static av_always_inline int bytestream2_seek_p(PutByteContext *p, int offset,
  ...
  switch (whence) {
    case SEEK_CUR:
      if (p->buffer_end - p->buffer < offset) p->eof = 1;
    ...
    case SEEK_END:
      ...
      break;
    case SEEK_SET:
      if (p->buffer_end - p->buffer_start < offset) p->eof = 1;
...
static av_always_inline unsigned int bytestream2_put_bufferu(
    PutByteContext *p, const uint8_t *src, unsigned int size) {
...
static av_always_inline unsigned int bytestream2_copy_bufferu(
    PutByteContext *p, GetByteContext *g, unsigned int size) {
...
static av_always_inline unsigned int bytestream2_copy_buffer(
    PutByteContext *p, GetByteContext *g, unsigned int size) {
  unsigned int size2;
